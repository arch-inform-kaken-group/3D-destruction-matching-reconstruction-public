...
# -*- coding: utf-8 -*-
"""
demo_plane_cut_v10.py — plane-cut fragmentation for pottery scans (v10).

Builds on v9 (grazing filter, U-gate closure, envelope clip, exact
triangle/plane intersection, nearest-2-seeds ownership, turning-angle
chaining, polygon/annulus scanline, open-shell GLBs) and adds:

A. MODEL-READY ADJACENCY DATA
   adjacency.json: nodes (fragment id, seed index, centroid, explode offset,
   area, face/point counts, palette color) + edges (join relationships):
   interface plane normal/midpoint, interface segment count & length,
   fill point count, fill area, mean wall thickness at interface, has_fill,
   and ground-truth relative translation between exploded fragments
   (rotation is identity in this pipeline).

B. PER-FRAGMENT DISTINCT COLORS in assembled PLY/GLB, ORIGINAL COLORS in exploded PLY/GLB
   (golden-angle HSV palette for assembled view to visualize joins;
   original realistic surface + fracture colors for exploded view and training).

C. REALISM UPGRADES
   1. world-unit wall thickness (inward voxel ray-march) + (t_ref/t)^alpha gain;
   2. band-limited stochastic stress multiplier exp(sigma * noise);
   3. feature-aware seed bias (rim proximity, curvature, thinness);
   4. stochastic NMS (per-candidate jittered exclusion radius + score noise);
   5. power-diagram weights (log-normal size factors) -> variable shard sizes
      while interfaces remain exact planes (squared-distance weights);
   6. shared deterministic FBM fracture relief (Hurst H, octaves, grain)
      displaced along the plane normal, faded to 0 at the intersection curve;
   7. micro-chipping of fracture rims (noise-gated boundary erosion);
   8. depth-based cross-section coloring (oxidized margins / reduced core);
   9. full parameter + statistics persistence in metadata.json.
...
DEFAULT_GRAY = 0.72
CLAY = np.array([0.62, 0.47, 0.36])
CORE = np.array([0.35, 0.30, 0.27])
...
class PointCloud:
    points: np.ndarray
    colors: np.ndarray
    def __len__(self): return int(len(self.points))
...
def bbox_diagonal(m):
    lo, hi = m.bounds; return float(np.linalg.norm(hi - lo))
def norm01(x):
    x = np.asarray(x, float); m = x.max()
    return x / m if m > 1e-12 else np.zeros_like(x)
def random_unit_vector(rng):
    v = rng.normal(size=3); n = float(np.linalg.norm(v))
...
def normalize_colors(arr, n):
    ...
    return np.clip(np.nan_to_num(a, nan=DEFAULT_GRAY, posinf=1.0, neginf=0.0), 0.0, 1.0)
...
def capture_source_texture(mesh):
    ...
    try:
        ...
        img = getattr(vis.material, "image", None) or getattr(vis.material, "baseColorTexture", None)
        if uv.ndim != 2 or uv.shape[0] != len(mesh.vertices) or uv.shape[1] != 2: return None, None
...
def double_sided_texture_material(image):
    return trimesh.visual.material.PBRMaterial(baseColorTexture=image, metallicFactor=0.0,
                                               roughnessFactor=0.95, doubleSided=True)


def ramp(t):
    t = np.clip(t, 0, 1)
    return np.stack([np.clip(1.6*t, 0, 1), np.clip(1.5*t-0.45, 0, 1)*0.7,
                     np.clip(1.0-1.4*t, 0, 1)*0.85 + 0.15*(1-t)], -1)


def vertex_field(mesh, face_vals):
    acc = np.zeros(len(mesh.vertices)); cnt = np.zeros(len(mesh.vertices))
...
def stress_textured_mesh(mesh, s_face):
    ...
    m = mesh.copy()
    if HAVE_PIL:
        grad = (np.clip(ramp(np.linspace(0.0, 1.0, 256)), 0, 1) * 255).astype(np.uint8)
...
def fem_lite(occ, iters, up_axis):
    S = occ.shape
    u = np.zeros((3,) + S); f = np.zeros((3,) + S)
    f[up_axis] = -1.0 * occ
    proj = occ.any(axis=tuple(i for i in range(3) if i != up_axis))
    z0 = int(np.where(proj)[0].min()) if proj.any() else 0
    idx = [slice(None)] * 3; idx[up_axis] = slice(z0, z0 + 3)
    mask = np.zeros(S, bool); mask[tuple(idx)] = True
    fixed = occ & mask
...
def wall_thickness_world(mesh, occ, T, max_steps=160, step=0.5):
    ...
    for k in range(1, int(max_steps) + 1):
        ...
        valid = ((gi >= 0).all(1) & (gi[:, 0] < S[0]) & (gi[:, 1] < S[1]) & (gi[:, 2] < S[2]))
        ins = np.zeros(F, bool)
        ix = np.flatnonzero(valid)
        if len(ix):
            ins[ix] = occ[gi[ix, 0], gi[ix, 1], gi[ix, 2]]
        start = (state == 0) & ins
        state[start] = 1; thick[start] += step
...
def curvature_field(mesh, radius):
    try:
        k = trimesh.curvature.discrete_mean_curvature_measure(mesh, mesh.triangles_center, radius)
...
def compute_stress(mesh, args, diag):
    ...
    out = {"s_hat": np.zeros(len(mesh.faces)), "thick": None, "t_ref": None,
    ...
    try:
        ...
        occ = None; vol = None; thick = None
    curv = norm01(curvature_field(mesh, diag / 40.0))
    if vol is None:
        if args.stress_model == "impact":
            dv = np.asarray(args.impact_dir, float); dv /= np.linalg.norm(dv) + 1e-12
            ip = mesh.bounds.mean(0) + dv * diag * 0.5
            _, iv, _ = trimesh.proximity.closest_point(mesh, ip[None])
            d = np.linalg.norm(mesh.triangles_center - mesh.triangles_center[iv[0]], axis=1)
            ...
            lever = np.linalg.norm(np.delete(mesh.triangles_center, up, 1) - c2, axis=1)
...
def nms_maxima(values, centers, radius, n, rng=None, radius_sigma=0.0, score_noise=0.0):
    ...
    if rng is not None and (radius_sigma > 0 or score_noise > 0):
        ...
        for i in order:
            if len(sel) >= n: break
            p = centers[i]; r = radii[i]
            ok = True
            for q, rq in zip(sel_pts, sel_rad):
                if np.linalg.norm(p - q) < 0.5 * (r + rq):
                    ok = False; break
            if ok:
                sel.append(int(i)); sel_pts.append(p); sel_rad.append(r)
        return np.asarray(sel, dtype=int)
    order = np.argsort(-values); sel = []
    for i in order:
        if len(sel) >= n: break
...
def sample_surface_points_colors(mesh, colors, n, rng):
    ...
    tot = float(areas.sum())
    p = areas / tot if tot > 1e-18 else np.full(len(mesh.faces), 1.0 / len(mesh.faces))
    fi = rng.choice(len(mesh.faces), size=n, p=p)
    r = rng.random((n, 2)); over = r.sum(1) > 1.0; r[over] = 1.0 - r[over]
...
def make_plane_basis(normal):
    ...
    u = np.cross(normal, h); u /= np.linalg.norm(u) + 1e-12
...
def plane_segments(mesh, face_idx, nv, mid, eps=1e-9):
    ...
    op = np.abs(d) <= eps
    ea = np.array([0, 1, 2]); eb = np.array([1, 2, 0])
    da = d[:, ea]; db = d[:, eb]
    ...
    if len(m2):
        ...
        good = L > eps
        if good.any():
            out.append(segs[good]); onrm.append(nrm[m2][good])
    for mi in np.flatnonzero(k > 2):
        ...
        if np.linalg.norm(p1 - p0) > eps:
            out.append(np.asarray([[p0, p1]], dtype=float)); onrm.append(nrm[mi][None, :])
    if not out:
        return np.zeros((0, 2, 3)), np.zeros((0, 3))
...
def _chain_polylines(segs2, snap):
    ...
    def nid(p):
        ...
        if k not in keys:
            keys[k] = len(node); node.append(np.asarray(p, float))
        return keys[k]
    for a, b in segs2:
        ia, ib = nid(a), nid(b)
    ...
    for eid, (a, b) in enumerate(edges):
        adj[a].append((eid, b)); adj[b].append((eid, a))
    ...
    for start_eid, (a, b) in enumerate(edges):
        ...
        if not closed:
            ...
            while True:
                ...
                if nb == seq[-1]:
                    closed = True; break
                seq.insert(0, nb); prev, cur = cur, nb
        if len(seq) >= 3:
...
def _point_in_poly(poly, p):
    ...
    cond = (y > p[1]) != (y2 > p[1])
    xt = x + (p[1] - y) * (x2 - x) / np.where(np.abs(y2 - y) < 1e-300, 1e-300, y2 - y)
...
def _scanline_mask(poly2, gs, xmin, ymin, rows, cols):
    px = (poly2[:, 0] - xmin) / gs
    py = (poly2[:, 1] - ymin) / gs
    x1 = np.roll(px, -1); y1 = np.roll(py, -1)
    ...
    rid = np.concatenate(rows_list); xv = np.concatenate(xs_list)
    order = np.lexsort((xv, rid)); rid = rid[order]; xv = xv[order]
    ...
    for r, xs in zip(np.split(rid, starts), np.split(xv, starts)):
        ...
        for a, b in zip(xs[0::2], xs[1::2]):
            i0 = max(0, int(np.ceil(a - 0.5))); i1 = min(cols - 1, int(np.floor(b - 0.5)))
...
def _value_noise_2d(shape, g, rng):
    ...
    y1 = np.minimum(y0 + 1, g - 1); x1 = np.minimum(x0 + 1, g - 1)
...
def fbm2(shape, rng, H, octaves, base_g=4):
    val = np.zeros(shape); amp = 1.0; g = base_g; tot = 0.0
    for _ in range(int(octaves)):
        val += amp * _value_noise_2d(shape, g, rng); tot += amp
        amp *= 2.0 ** (-float(H)); g *= 2
    val -= val.mean()
    s = val.std()
    return val / s if s > 1e-9 else val
...
# ============================ cross-section fill ============================
def plane_cross_fill(mesh, face_idx, nv, mid, spacing, fill_color, rng,
                     seeds_all, weights_all, si, sj, diag, tag="",
                     max_fill_points=200_000, tree_seeds=None,
                     height_cfg=None, color_cfg=None, chip_cfg=None):
    ...
    def dbg(m):
        if tag: log(f"      {tag}: {m}")
    stats = {"n_raw": 0, "n_seeds": 0, "n_kept": 0, "n_open": 0, "n_closed": 0,
             "matched_open": 0, "matched_closed": 0, "fill_px": 0, "fill_area": 0.0,
             "interface_length": 0.0, "height_rms": 0.0, "has_fill": False,
             "reason": ""}
    segs, norms = plane_segments(mesh, face_idx, nv, mid)
    stats["n_raw"] = len(segs)
    if len(segs) < 2:
        stats["reason"] = f"segments={len(segs)}"
        dbg(f"NO FILL ({stats['reason']})"); return None, None, stats
    ...
    for a in range(len(closed_p)):
        for b in range(a + 1, len(closed_p)):
            ...
            m = (mo & ~mi) & binary_dilation(envelope, _disk(sep_c / (2 * gs) + 2))
            if m.sum() >= 16:
                fill |= m; used_c.add(a); used_c.add(b); n_mc += 1
    for a in range(len(closed_p)):
    ...
    for a in range(len(open_p)):
        for b in range(a + 1, len(open_p)):
            ...
            proximity = max(float(np.percentile(dAB, 90)), float(np.percentile(dBA, 90)))
    ...
    for endpoint, proximity, a, b, ff in cand:
        ...
        used_o.add(a); used_o.add(b)
        A = open_p[a]; B = open_p[b] if ff else open_p[b][::-1]
    ...
    if chip_cfg is not None and fill.any():
        ...
        p_chip = chip_cfg["prob"] * np.exp(-edge / max(1e-9, chip_cfg["width_px"]))
        fill = fill & ~(rng.random(fill.shape) < p_chip)
    fy, fx = np.where(fill)
    if len(fx) < 16:
        stats["reason"] = (f"segs={stats['n_raw']} seeds={stats['n_seeds']} kept={stats['n_kept']} "
    ...
    pts = (mid
           + (xmin + (fx + 0.5 + jx) * gs)[:, None] * u
           + (ymin + (fy + 0.5 + jy) * gs)[:, None] * v)
    # --- shared fractal relief, faded to 0 at the intersection curve (v10) ---
    h = np.zeros(len(fx))
    if height_cfg is not None:
        edge = distance_transform_edt(fill)
        fade = np.clip(edge[fy, fx] / max(1e-9, height_cfg["fade_px"]), 0.0, 1.0) ** 0.7
        base_g = int(np.clip(min(rows, cols) / max(4.0, height_cfg["wavelength_px"]), 2, 64))
        field = fbm2((rows, cols), height_cfg["rng"], height_cfg["H"],
                     height_cfg["octaves"], base_g=base_g)
        grain = height_cfg["rng"].standard_normal(len(fx))
        h = (height_cfg["amp"] * field[fy, fx] + height_cfg["grain"] * grain) * fade
        pts = pts + h[:, None] * nv
    ...
    if color_cfg is not None and color_cfg.get("model") == "depth_core":
        ...
        depth = np.clip(d_surf / (0.5 * max(color_cfg["t_local"], 1e-9)), 0.0, 1.0)
        ...
        cols_arr = np.tile(np.clip(np.asarray(fill_color, float), 0, 1), (len(pts), 1))
    stats["has_fill"] = True
...
def process_file(in_path, out_root, args):
    ...
    sidx = nms_maxima(score, P, diag / (2.2 * max(args.fragments, 2) ** (1 / 3)),
                      args.fragments, rng=rng, radius_sigma=args.seed_radius_sigma,
    ...
    corners = np.array([[x, y, z] for x in (lo_b[0], hi_b[0])
                        for y in (lo_b[1], hi_b[1]) for z in (lo_b[2], hi_b[2])])
    boundary_faces = {}
    for i in range(nS):
    ...
    for (i, j), face_idx in sorted(boundary_faces.items()):
        ...
        height_cfg = dict(rng=rng_child(args.seed, 300 + i, 400 + j),
    ...
    palette = fragment_palette(max(nS, 1), sat=args.palette_sat, val=args.palette_val)
    label_faces = [np.where(face_labels == k)[0] for k in range(nS)]
    faces_all = np.asarray(mesh.faces); verts_all = np.asarray(mesh.vertices)
    ...
    for i in range(nS):
        ...
        dv = centroid - center; nn = float(np.linalg.norm(dv))
        ...
        vis_cols = shade_palette(palette[i], sub_cols, args.assembly_color_mode)
        asm_shell = apply_vertex_colors(asm_shell, vis_cols)
        
        m = pc_labels == i
        pts, cols = pc_pts[m], pc_cols[m]
        for fp, fc in fill_dict.get(i, []):
            pts = np.vstack([pts, fp]); cols = np.vstack([cols, fc])
        ...
        seed_to_fid[i] = fid
        fragments.append(Fragment(fid=fid, seed_index=i, shell=shell, exploded_shell=ex_shell,
                                  centroid=centroid, explode_offset=off,
                                  n_faces=len(shell.faces), area=float(shell.area),
                                  n_points=int(len(pts)), palette_rgb=palette[i],
                                  vert_colors=sub_cols))

    def save_pc(pts, cols, path):
        rgba = np.column_stack([(np.clip(cols, 0, 1) * 255).astype(np.uint8),
                                np.full((len(cols), 1), 255, dtype=np.uint8)])
        trimesh.PointCloud(np.asarray(pts, float), colors=rgba).export(str(path))

    def rel(p): return str(Path(p).relative_to(out))
    files = {}
    save_pc(pc_pts, pc_cols, out / "complete.ply");             files["complete_pc"] = rel(out / "complete.ply")
    save_pc(np.vstack(asm_p), np.vstack(asm_c), out / "assembled.ply"); files["assembled_pc"] = rel(out / "assembled.ply")
    save_pc(np.vstack(exp_p), np.vstack(exp_c), out / "exploded.ply");  files["exploded_pc"] = rel(out / "exploded.ply")
    for f in fragments:
        ...
        for fp, fc in fill_dict.get(f.seed_index, []):
            pts = np.vstack([pts, fp]); cols = np.vstack([cols, fc])
        pp = fdir / f"{f.fid}.ply"
        save_pc(pts, cols, pp); f.ply = rel(pp)
    trimesh.Scene(assembled).export(str(out / "assembled.glb")); files["assembled_mesh"] = rel(out / "assembled.glb")
    trimesh.Scene(exploded).export(str(out / "exploded.glb"));   files["exploded_mesh"] = rel(out / "exploded.glb")
    stress_textured_mesh(mesh, s_hat).export(str(out / "stress_colored.glb")); files["stress_mesh"] = rel(out / "stress_colored.glb")

    # ---- adjacency / join-relationship data for matching+reconstruction models ----
    nodes = []
    for f in fragments:
        nodes.append({"id": f.fid, "seed_index": int(f.seed_index),
                      "centroid": f.centroid.tolist(),
                      "explode_offset": f.explode_offset.tolist(),
                      "assembled_transform": {"rotation": np.eye(3).tolist(),
                                              "translation": [0.0, 0.0, 0.0]},
                      "exploded_transform": {"rotation": np.eye(3).tolist(),
                                             "translation": f.explode_offset.tolist()},
                      "n_faces": int(f.n_faces), "area": float(f.area),
                      "n_points": int(f.n_points),
                      "palette_rgb": np.round(f.palette_rgb, 4).tolist(),
                      "ply": f.ply})
    ...
    for (i, j), st in sorted(pair_stats.items()):
        ...
        edges.append({"a": fi.fid, "b": fj.fid, "seed_pair": [int(i), int(j)],
                      "plane_normal": st["plane_normal"],
                      "plane_midpoint": st["plane_midpoint"],
                      "interface_segments": int(st["n_kept"]),
                      "interface_length": float(st["interface_length"]),
                      "fill_points": int(st["fill_px"]) if st["has_fill"] else 0,
                      "fill_area": float(st["fill_area"]) if st["has_fill"] else 0.0,
                      "mean_wall_thickness": float(st["t_local"]),
                      "height_rms": float(st["height_rms"]),
                      "has_fill": bool(st["has_fill"]),
                      "rel_rotation": np.eye(3).tolist(),
                      "rel_translation_exploded": (fj.explode_offset - fi.explode_offset).tolist()})
        neighbors[fi.fid].append(fj.fid); neighbors[fj.fid].append(fi.fid)
    adjacency = {"object": stem, "n_fragments": len(fragments),
                 "n_edges": len(edges), "nodes": nodes, "edges": edges}
    (out / "adjacency.json").write_text(json.dumps(adjacency, indent=2, default=str))
    files["adjacency"] = rel(out / "adjacency.json")

    areas = np.array([f.area for f in fragments]) if fragments else np.zeros(0)
    meta = {"input": str(in_path),
            "pipeline": "plane_cut_v10_adjacency_palette_relief",
            "seed": int(args.seed), "n_fragments": len(fragments),
            "point_spacing": spacing, "fill_points": int(n_fill_total),
            "thickness": ({"median": float(t_ref),
                           "p10": float(np.quantile(thick[thick > 0], 0.10)),
                           "p90": float(np.quantile(thick[thick > 0], 0.90))}
                          if thick is not None and t_ref else None),
            "fragment_areas": ({"mean": float(areas.mean()), "std": float(areas.std()),
                                "cv": float(areas.std() / max(areas.mean(), 1e-12))}
                               if len(areas) else None),
            "args": vars(args), "files": files,
            "fragments": [{"id": f.fid, "ply": f.ply, "n_faces": f.n_faces,
                           "area": f.area, "n_points": f.n_points,
                           "palette_rgb": np.round(f.palette_rgb, 4).tolist(),
                           "neighbors": neighbors.get(f.fid, [])} for f in fragments]}
    ...
    return {"input": str(in_path), "output": str(out),
            "n_fragments": len(fragments), "adjacency_edges": len(edges), "status": "ok"}
...
def build_parser():
    ...
    p.add_argument("--output", "-o", default=r"D:\storage\jomon_kaen\fragmented_plane_cut", type=Path)
    ...
    g.add_argument("--stress-model", choices=["fem_lite", "impact", "gravity", "curvature", "none"], default="fem_lite")
    g.add_argument("--grid-res", type=int, default=64)
    g.add_argument("--jacobi-iters", type=int, default=150)
    g.add_argument("--impact-dir", type=float, nargs=3, default=[1.0, 0.0, 0.35])
    ...
    g.add_argument("--thickness-alpha", type=float, default=1.5, help="stress gain (t_ref/t)^alpha")
    g.add_argument("--thickness-cap", type=float, default=5.0)
    g.add_argument("--noise-sigma", type=float, default=0.5, help="stochastic stress multiplier sigma")
    ...
    g.add_argument("--size-sigma", type=float, default=0.6, help="log-normal size sigma (0=uniform)")
    ...
    g.add_argument("--min-faces", type=int, default=20)
...
def main(argv=None):
    ...
    files = ([args.input] if args.input.is_file() else
             sorted(p for p in args.input.rglob("*")
    ...
    for k, f in enumerate(files, 1):
        ...
        try:
            ...
            sums.append({"input": str(f), "n_fragments": 0, "status": f"error: {exc}"})
