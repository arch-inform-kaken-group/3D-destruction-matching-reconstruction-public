...
# -*- coding: utf-8 -*-
"""
visualize_pipeline.py — stage-by-stage visualization of the v10 pipeline (v6).

v6 changes:
  * Stage 10 (Terrain) redesigned to feature a clean 1x2 layout showing a
    2D cut-plane projection of the fragment + relief, and a 3D view of the
    single isolated fragment to explicitly expose the fracture surface.
...
try:
    import stress_v4 as pipe
except ImportError:
    print("ERROR: save the v10 pipeline as 'stress_v4.py' "
          "in the same directory.", file=sys.stderr)
    sys.exit(1)
...
def clean3d(ax, lo=None, hi=None, title="", view_dir=None, view_angles=None):
    """Orthographic, equal aspect, no axes/grid."""
    ax.set_axis_off()
    ax.grid(False)
    try:
...
def get_user_view_mesh(mesh, title="Set Camera Angle"):
    ...
    ax.plot_trisurf(v[:, 0], v[:, 1], v[:, 2], triangles=f, color='lightgray', alpha=0.9, edgecolor='none')
    clean3d(ax, v.min(0), v.max(0), f"{title}\n(Rotate with mouse, then close the window to confirm)")
    
    view_state = {'elev': ax.elev, 'azim': ax.azim, 'roll': getattr(ax, 'roll', 0)}
    def on_draw(event):
    ...
    plt.show(block=True)
    print(f"    -> Captured global view: elev={view_state['elev']:.1f}°, azim={view_state['azim']:.1f}°")
...
def get_user_view_plane(pc_pts, segs, mid, u, v, seeds, pair, title):
    ...
    ax.scatter(psub[:,0], psub[:,1], psub[:,2], s=0.5, c='gray', alpha=0.3, depthshade=False)
    ...
    if len(segs):
        ...
        c00 = mid + umin * u + vmin * v; c10 = mid + umax * u + vmin * v
        c11 = mid + umax * u + vmax * v; c01 = mid + umin * u + vmax * v
        ax.add_collection3d(Poly3DCollection(
            [np.array([c00, c10, c11]), np.array([c00, c11, c01])],
            alpha=0.30, facecolor="#4da3ff", edgecolor="none"))
    ...
    clean3d(ax, pc_pts.min(0), pc_pts.max(0), f"{title}\n(Rotate with mouse, then close the window to confirm)")
    ...
    fig.canvas.mpl_connect('draw_event', on_draw)
    plt.show(block=True)
    print(f"    -> Captured plane view: elev={view_state['elev']:.1f}°, azim={view_state['azim']:.1f}°")
    return view_state['elev'], view_state['azim'], view_state['roll']
...
def viz_01_input(pc_pts, pc_cols, outdir, view_angles):
    ...
    clean3d(ax, pc_pts.min(0), pc_pts.max(0),
            f"Stage 1: Original Artifact Point Cloud Representation ({len(pc_pts):,} pts)", 
...
def viz_02_density(pc_pts, pc_cols, outdir, view_angles):
    ...
    clean3d(ax, pts.min(0), pts.max(0),
            "Stage 2: Surface Sampling Density and Spatial Uniformity (6-NN)",
            view_angles=view_angles)
    save(fig, outdir / "02_point_cloud.png")
    rgba = np.column_stack([(np.clip(pc_cols, 0, 1) * 255).astype(np.uint8),
                            np.full((len(pc_cols), 1), 255, dtype=np.uint8)])
    trimesh.PointCloud(pc_pts, rgba).export(
...
def viz_04_thickness(pc_pts, thick_f, outdir, view_angles):
    ...
    pc(ax, pc_pts, cols)
    clean3d(ax, pc_pts.min(0), pc_pts.max(0),
            "Stage 3b: Volumetric Wall Thickness Estimation (World Units)",
            view_angles=view_angles)
    save(fig, outdir / "04_wall_thickness.png")


def viz_05_seeds(pc_pts, score_f, seeds, outdir, view_angles):
    cols = cmap("hot")(np.clip(score_f, 0, 1))[:, :3]
    psub, csub = _sub(pc_pts, cols, 100_000)
    fig = plt.figure(figsize=(7, 6))
    ax = fig.add_subplot(projection="3d")
    pc(ax, psub, csub * 0.85, s=0.35, n=10**9)          # stress underlay
    ax.scatter(seeds[:, 0], seeds[:, 1], seeds[:, 2], c="lime", s=95,
...
def viz_07_plane(pc_pts, pc_cols, segs, nv, mid, u, v, seeds, pair, outdir, view_angles):
    i, j = pair
    fig = plt.figure(figsize=(13, 6.5))

    ax1 = fig.add_subplot(1, 2, 1, projection="3d")
    psub, csub = _sub(pc_pts, pc_cols, 60_000)
    dim = np.column_stack([csub * 0.35 + 0.45, np.full(len(csub), 0.22)])
    pc(ax1, psub, dim, s=0.25, n=10**9)
    if len(segs):
        rel = segs.reshape(-1, 3) - mid
        xy = np.stack([rel @ u, rel @ v], -1).reshape(-1, 2, 2)
        umin, umax = xy[..., 0].min(), xy[..., 0].max()
        ...
        ax1.add_collection3d(Poly3DCollection(
            ...
            alpha=0.30, facecolor="#4da3ff", edgecolor="none"))
        for a_, b_ in [(c00, c10), (c10, c11), (c11, c01), (c01, c00)]:
            ax1.plot(*zip(a_, b_), color="#08306b", lw=1.4)
        for sgm in segs:
            ax1.plot(sgm[:, 0], sgm[:, 1], sgm[:, 2], color="red", lw=2.4)
            
    ax1.scatter(*seeds[i], c="lime", s=90, edgecolors="k", depthshade=False)
    ax1.scatter(*seeds[j], c="orange", s=90, edgecolors="k", depthshade=False)
    clean3d(ax1, pc_pts.min(0), pc_pts.max(0),
            f"Stage 7: Cut Plane Intersection {i}|{j} \n({len(segs)} Kept Segments)", 
            view_angles=view_angles)
    ...
    if len(segs):
        sp = segs.reshape(-1, 3)
        rgba = np.full((len(sp), 4), [255, 0, 0, 255], dtype=np.uint8)
        trimesh.PointCloud(sp, rgba).export(
            str(outdir / "samples" / "stage07_plane_segments.ply"))
...
def viz_08_fill(segs, nv, mid, u, v, spacing, diag, outdir):
    ...
    xy = np.stack([rel @ u, rel @ v], -1).reshape(-1, 2, 2)
    margin = 2.0 * spacing
    xmin = xy[..., 0].min() - margin; xmax = xy[..., 0].max() + margin
    ymin = xy[..., 1].min() - margin; ymax = xy[..., 1].max() + margin
    gs = spacing
    cols = int((xmax - xmin) / gs) + 1
    rows = int((ymax - ymin) / gs) + 1
    if cols > 2000 or rows > 2000:
        sc = max(cols / 2000, rows / 2000); gs *= sc
        cols = int((xmax - xmin) / gs) + 1
        rows = int((ymax - ymin) / gs) + 1
    if cols < 3 or rows < 3:
        return None
    ...
    for a in range(len(closed_p)):
        for b in range(a + 1, len(closed_p)):
            ...
            if pipe._point_in_poly(L2, L1[0]) and not pipe._point_in_poly(L1, L2[0]):
                outer, inner = L2, L1
            elif pipe._point_in_poly(L1, L2[0]) and not pipe._point_in_poly(L2, L1[0]):
                outer, inner = L1, L2
            else:
                continue
            ...
            m = (mo & ~mi) & binary_dilation(envelope, pipe._disk(sep_c / (2 * gs) + 2))
            if m.sum() >= 16:
                fill |= m; used_c.add(a); used_c.add(b)
    for a in range(len(closed_p)):
        if a in used_c:
            continue
    ...
    for a in range(len(open_p)):
        for b in range(a + 1, len(open_p)):
            ...
            dAB = cKDTree(B).query(A)[0]; dBA = cKDTree(A).query(B)[0]
            prox = max(float(np.percentile(dAB, 90)), float(np.percentile(dBA, 90)))
            ep = min(d_ff, d_fr)
    ...
    for a in range(len(open_p)):
        ...
        if len(A) < 3:
            continue
        ed = float(np.linalg.norm(A[0]-A[-1])); pl = pipe._poly_length(A)
    ...
    axes[0].imshow(envelope, cmap="gray_r", origin="lower", interpolation="nearest")
    axes[0].set_title("Stage 8A: Bounding Envelope of Kept Segments", fontsize=10, fontweight="bold")
    ...
    axes[1].set_title(f"Stage 8B: Rasterized Solid Fill Mask ({int(fill.sum())} px)", fontsize=10, fontweight="bold")
    
    for p, c in polys:
        px = (p[:, 0] - xmin) / gs; py = (p[:, 1] - ymin) / gs
        axes[2].plot(px, py, "r-" if c else "b-", lw=1.0, alpha=0.8)
    axes[2].set_title(f"Stage 8C: Chained Vector Polylines\n(Closed={len(closed_p)}, Open={len(open_p)})",
                      fontsize=10, fontweight="bold")
    for ax in axes:
        clean2d(ax)
    fig.tight_layout()
...
def get_user_view_fragment(csub, psub, title="Set Fragment 3D View"):
    ...
    ax.scatter(csub[:, 0], csub[:, 1], csub[:, 2], s=0.3, c='gray', alpha=0.3, depthshade=False)
    ax.scatter(psub[:, 0], psub[:, 1], psub[:, 2], s=1.0, c='saddlebrown', alpha=0.8, depthshade=False)
    
    clean3d(ax, csub.min(0), csub.max(0), f"{title}\n(Rotate with mouse, then close window to confirm)")
    ...
    fig.canvas.mpl_connect('draw_event', on_draw)
    plt.show(block=True)
    print(f"    -> Captured fragment view: elev={view_state['elev']:.1f}°, azim={view_state['azim']:.1f}°")
    return view_state['elev'], view_state['azim'], view_state['roll']


# ── fracture terrain (5-Subplot Layout with Interactive Fragment View) ──
def viz_09_terrain(fill_result, t_local, spacing, outdir, args, rng_seed,
                   pc_pts, pc_labels, pair):
    ...
    field = pipe.fbm2((rows, cols), h_rng, args.height_H, args.height_octaves, base_g=base_g)
    ...
    fragment_view = get_user_view_fragment(csub, psub, title=f"Set 3D View for Fragment {i}")
    ...
    sc = ax_c.scatter(u_coords, v_coords, c=h_vals, cmap="terrain", s=2.5, alpha=0.9, rasterized=True)
    
    ax_c.set_aspect('equal')
    clean2d(ax_c)
    ax_c.set_title(f"2D Projection: Relief on Fragment {i}", fontsize=10, fontweight="bold")
    cbar = fig.colorbar(sc, ax=ax_c, shrink=0.7, pad=0.02)
    cbar.set_label("Displacement (World Units)", fontsize=8)
    ...
    clean3d(ax_d, allp.min(0), allp.max(0),
            f"3D View: Procedural Relief on Fragment {i}", view_angles=fragment_view)
    ...
    ax_e.set_title(f"Cross-section Relief Profile (Row {rows // 2})",
                   fontsize=10, fontweight="bold")
    clean2d(ax_e, keep_spines=True)

    fig.suptitle("Stage 10: Procedural Fracture Relief Synthesis via FBM", fontsize=15, fontweight="bold", y=0.96)
...
def viz_10_adjacency(fragments, pair_stats, node_by_seed, outdir):
    ...
    for (i, j), st in pair_stats.items():
        if st.get("n_kept", 0) < 1 or i not in node_by_seed or j not in node_by_seed:
            continue
        a = id_to_idx[node_by_seed[i].fid]; b = id_to_idx[node_by_seed[j].fid]
        key = (min(a, b), max(a, b))
        if key in seen:
            continue
        seen.add(key)
        ax.plot(pos[[a, b], 0], pos[[a, b], 1], color="gray", lw=0.9, alpha=0.55, zorder=1)
    for k in range(len(fragments)):
        ax.scatter(pos[k, 0], pos[k, 1], c=[palette[k]], s=200,
                   edgecolors="black", linewidths=0.7, zorder=3)
        ax.annotate(fragments[k].fid, (pos[k, 0], pos[k, 1]), fontsize=6,
                    ha="center", va="bottom", xytext=(0, 7), textcoords="offset points")
    ax.set_aspect("equal")
    clean2d(ax)
    ax.set_title(f"Stage 12: Topological Adjacency Network\n({len(fragments)} Nodes, {len(seen)} Edges)",
...
def viz_11_final(asm_p, asm_c, exp_p, exp_c, mesh, outdir, view_angles):
    ...
    clean3d(ax1, lo, hi, "Stage 11: Final Virtual Assembly\n(Distinct Palette Colors & Generated Fills)", view_angles=view_angles)
    
    pad = np.linalg.norm(hi - lo) * 0.18
    pc(ax2, np.vstack(exp_p), np.vstack(exp_c))
    clean3d(ax2, lo - pad, hi + pad, "Stage 11: Exploded Fragmentation View\n(Original Textures & Geometries)", view_angles=view_angles)
...
def main(argv=None):
    p = argparse.ArgumentParser(description="Pipeline stage visualizer (v10, interactive point-cloud renders)")
    ...
    args = pipe.build_parser().parse_args([
        "--input", str(cli.input), "--output", str(Path(cli.output) / "_tmp"),
        "--fragments", str(cli.fragments), "--seed", str(cli.seed),
    ...
    spacing = float(np.sqrt(area / max(len(pc_pts), 1))) if area > 1e-18 else diag / 1000
    ...
    if thick is not None and t_ref:
        score *= pipe.thickness_gain(thick, t_ref, args.thickness_alpha, args.thickness_cap)
    if args.noise_sigma > 0:
        base_shard = diag / max(args.fragments, 2) ** (1 / 3)
        noise = pipe.bandlimited_noise(P, pipe.rng_child(args.seed, 101),
    ...
    if thick is not None:
        ...
        thin_bias = 1 + args.thin_bias_weight * pipe.norm01(1 / np.clip(thick, t_floor, None))
    else:
    ...
    viz_04_thickness(pc_pts, thick[pc_fidx] if thick is not None else None, outdir, global_view)

    print("[viz] Stage 4-5: seeds + labels")
    sidx = pipe.nms_maxima(score, P, diag / (2.2 * max(args.fragments, 2) ** (1 / 3)),
                           args.fragments, rng=rng, radius_sigma=args.seed_radius_sigma,
                           score_noise=args.seed_score_noise)
    seeds = P[sidx]; nS = len(seeds)
    viz_05_seeds(pc_pts, score_pts, seeds, outdir, global_view)
    if args.size_sigma > 0 and nS > 1:
        base_sp = diag / max(args.fragments, 2) ** (1 / 3)
        factors = pipe.rng_child(args.seed, 202).lognormal(0, args.size_sigma, nS)
    ...
    for i in range(nS):
        for j in range(i + 1, nS):
            dv = seeds[j] - seeds[i]; dd = float(np.linalg.norm(dv))
            if dd < 1e-12:
                continue
            n = dv / dd
            mid = pipe.power_mid(seeds[i], seeds[j], weights[i], weights[j])
    ...
    if best_data is not None and len(best_data[4]) >= 2:
        ...
        plane_view = get_user_view_plane(pc_pts, segs, mid, u, v, seeds, best_pair, "Set Plane Cut Camera Angle")
        
        viz_07_plane(pc_pts, pc_cols, segs, nv, mid, u, v, seeds, best_pair,
                     outdir, view_angles=plane_view)
        ...
        # New 2-panel 2D/3D Terrain Generation
        viz_09_terrain(fill_result, t_local, spacing, outdir, args, args.seed,
                       pc_pts, pc_labels, best_pair)
    else:
        print("    ! no pair produced kept segments; skipping plane/terrain figures")
    ...
    for (i, j), fidx in sorted(boundary_faces.items()):
        nv = seeds[j] - seeds[i]; dd = float(np.linalg.norm(nv))
        if dd < 1e-12:
            continue
        nv /= dd
        mid = pipe.power_mid(seeds[i], seeds[j], weights[i], weights[j])
        ...
        hcfg = dict(rng=pipe.rng_child(args.seed, 300 + i, 400 + j), H=args.height_H,
                    octaves=args.height_octaves, amp=args.height_amp_scale * t_loc,
                    grain=args.grain_amp_scale * t_loc,
                    fade_px=max(2.0, args.height_fade_fraction * wall_px),
                    wavelength_px=1.5 * wall_px)
        ccfg = dict(model=args.fill_color_model, tree=tree_surf, t_local=t_loc,
                    surface_color=np.asarray(args.surface_color, float),
                    core_color=np.asarray(args.core_color, float))
        chcfg = dict(prob=args.chip_prob, width_px=max(1.0, args.chip_width_scale))
        ...
        st["t_local"] = t_loc; st["plane_normal"] = nv.tolist(); st["plane_midpoint"] = mid.tolist()
        pair_stats[(i, j)] = st
        if fp is None:
            continue
    ...
    palette = pipe.fragment_palette(max(nS, 1), sat=args.palette_sat, val=args.palette_val)
    ...
    for i in range(nS):
        ...
        asm_p.append(pts)
        asm_c.append(pipe.shade_palette(palette[i], cols, args.assembly_color_mode))
        exp_p.append(pts + off)
        exp_c.append(cols)
        fid = f"frag_{len(fragments):03d}"
        node_by_seed[i] = type("N", (), {"fid": fid, "centroid": centroid,
                                         "explode_offset": off})()
        fragments.append(type("Fr", (), {"fid": fid, "seed_index": i, "centroid": centroid,
                                         "n_faces": len(idx_f),
    ...
    if asm_p:
        ...
        rgba_a = np.column_stack([(np.clip(np.vstack(asm_c), 0, 1) * 255).astype(np.uint8),
                                  np.full((sum(len(a) for a in asm_p), 1), 255, dtype=np.uint8)])
        trimesh.PointCloud(np.vstack(asm_p), rgba_a).export(
            str(outdir / "samples" / "stage11_assembled.ply"))
        rgba_e = np.column_stack([(np.clip(np.vstack(exp_c), 0, 1) * 255).astype(np.uint8),
                                  np.full((sum(len(a) for a in exp_p), 1), 255, dtype=np.uint8)])
        trimesh.PointCloud(np.vstack(exp_p), rgba_e).export(
            str(outdir / "samples" / "stage11_exploded.ply"))
